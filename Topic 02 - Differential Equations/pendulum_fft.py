...
def solve_rk45(theta0, omega0, t_final, dt):
    """RK45 with fixed max step size for fair comparison."""
    sol = solve_ivp(
        _ode_model,
        (0, t_final),
        [omega0, theta0],
...
def solve_yoshida4(theta0, omega0, t_final, dt):
    ...
    for i in range(n_steps - 1):
        ...
        om = omega[i]
        th = th + c1 * om * dt
        om = om + d1 * angular_acceleration(th) * dt
        th = th + c2 * om * dt
        om = om + d2 * angular_acceleration(th) * dt
        th = th + c3 * om * dt
        om = om + d3 * angular_acceleration(th) * dt
        th = th + c4 * om * dt
        theta[i + 1] = th
        omega[i + 1] = om
    return t, theta, omega
...
def plot_energy_error(ax, t, abs_energy_error, color):
    """Plot |relative energy error| vs time on a log scale."""
...
def plot_power_spectrum(ax, freqs, power_db, color, f_max=3.0):
    """Plot power spectrum in dB up to f_max Hz."""
...
def compute_spectrum(theta, dt):
    """Compute normalized FFT power spectrum and frequency axis."""
    n = len(theta)
    ct = 2 / n * fft(theta)
