...
t = np.zeros(n, dtype=float)  # (Julian years)
...
for j in range(planet_count):
    ...
    ratio = T**2 / a**3
    print(f"{name:>9}: T= {T:>8.4f} yr,", end=" ")
    print(f"a= {a:>7.4f} AU, T^2/a^3= {ratio:.6f} yr^2/AU^3")
...
plt.title(f"Planet Orbits ({tf} Julian Earth Years)")
plt.xlabel("x (AU)")
plt.ylabel("y (AU)")
