...
If the notebook imports matplotlib anywhere, the very next line after the
docstring and the cell-label comment must select an inline backend:
...
notebook. ``cse101.code-workspace`` pins this with
...
Jupyter extension does not record them, and no notebook in this repository
carries any. `nb_clean.py` still strips them, because JupyterLab and `nbconvert
--execute` do write them, and a notebook that arrives from either would
otherwise carry timing noise into the index.
